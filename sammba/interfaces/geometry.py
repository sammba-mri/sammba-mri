...
class HistogramMaskInputSpec(BaseInterfaceInputSpec):
    ...
    opening = traits.Int(
        ...
        desc="Order of the morphological opening to perform, to keep only "
             ...
             "the image during the mask computation. [default: 4]",
        usedefault=True)
    closing = traits.Int(
        0,
...
class HistogramMask(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        n_voxels_min = self.inputs.volume_threshold * .9 / affine_det
        ...
        n_voxels_max = self.inputs.volume_threshold * 1.5 / affine_det
        opening = 0
        ...
        iterations = 0
        n_voxels_min = self.inputs.volume_threshold * .8 / affine_det
        while (n_voxels_mask < n_voxels_min) and (iterations <
                                                  self.inputs.closing):
            iterations += 1
            for structure_size in range(1, 4):
                structure = morphology.generate_binary_structure(
                    3, structure_size)
                mask_data = morphology.binary_closing(
                    mask_data, structure=structure, iterations=iterations)
                n_voxels_mask = np.sum(mask_data > 0)
                if self.inputs.verbose:
                    print('volume {0} after {1} closing iterations'.format(
                        n_voxels_mask * affine_det, iterations))
                if n_voxels_mask >= n_voxels_min:
                    break
        ...
        # Fill holes
        n_voxels_min = self.inputs.volume_threshold / affine_det
        if n_voxels_mask < n_voxels_min:
            for structure_size in range(1, 4):
                structure = morphology.generate_binary_structure(
                    3, structure_size)
                mask_data = morphology.binary_fill_holes(
        ...
        for n in range(3):
            if n_voxels_mask < n_voxels_min * 1.1:
                ...
                mask_data = morphology.grey_dilation(mask_data, size=size)
                n_voxels_mask = np.sum(mask_data > 0)
                if n_voxels_mask == previous_n_voxels_mask:
                    size2 = (size[0] * 2, size[1] * 2, size[2] * 2)
                    mask_data = morphology.grey_dilation(mask_data, size=size2)
                    n_voxels_mask = np.sum(mask_data > 0)

                if self.inputs.verbose:
                    print('volume {0}, grey dilation {1}, size {2}'.format(
                        n_voxels_mask * affine_det, n, size))
            else:
                break
