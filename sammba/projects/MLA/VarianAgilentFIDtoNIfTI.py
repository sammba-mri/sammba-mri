...
#%%
dic, data = ng.varian.read(fiddir, as_2d = 1)
...
image = np.empty((nro, npe, nslices, nblocks))
...
for block in range(nblocks):
    for slicen in pssorder:
        ...
        image[:, :, slicen, block] = np.absolute(
            np.fft.fftshift(np.fft.fft2(data[linepicker][peorder])))

image = np.average(image, axis = 3)
    
#%%

dx = float(dic['procpar']['lro']['values'][0]) * 10 / nro #in cm not mm!
dy = float(dic['procpar']['lpe']['values'][0]) * 10 / npe #in cm not mm!
...
img = nibabel.Nifti1Image(image, affine, header = header)
