...
analysisdir = os.path.join(projectdir, 'RsLemurs/analysis20171030')
...
SIAPfix = 'yes'
logfile = os.path.join(projectdir, 'RsLemurs', strftime("%Y%m%d_%H%M%S", gmtime()) + '_log.txt')
rminterfiles = 'yes'
...
RsLemurssessions = MRIsessions[(MRIsessions['date'] < '2017-09-26')]
PVdirs = [str(x) for x in RsLemurssessions.PVdir.tolist()]

#%%

for PVdir in PVdirs:
    infotab = RsLemurssessions[(RsLemurssessions.PVdir == PVdir)]
