...
def test_fmri_session_init():
    ...
    func_file = os.path.join(os.path.dirname(testing_data.__file__),
                             'func.nii.gz')
    data = fmri.FMRISession(anat=anat_file, func=func_file)
...
def test_fmri_session_check_inputs():
    ...
    data = fmri.FMRISession(anat='', func=func_file, animal_id='001')
    assert_raises_regex(IOError, "anat must be an existing image file",
                        data._check_inputs)

    data = fmri.FMRISession(anat=anat_file, func='', animal_id='001')
...
def test_coregister():
    ...
    animal_session = fmri.FMRISession(anat=anat_file, func=func_file,
                                      animal_id='test_coreg_dir')

    if afni.Info().version():
        tempdir = tempfile.mkdtemp()
        animal_session.coregister(animal_session, 1., tempdir, 400,
