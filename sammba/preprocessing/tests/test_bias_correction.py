...
from sammba.segmentation import bias_correction
...
def test_ants_n4():
    in_file = os.path.join(os.path.dirname(testing_data.__file__),
                           'anat.nii.gz')
    unbiased_file = bias_correction._ants_n4(in_file, write_dir=tst.tmpdir,
                                             verbose=False)
...
def test_afni_unifize():
    in_file = os.path.join(os.path.dirname(testing_data.__file__),
                           'anat.nii.gz')
    unbiased_file = bias_correction._afni_unifize(in_file,
